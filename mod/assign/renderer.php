        ...
        if (!$status->teamsubmissionenabled) {
            ...
            if ($status->teamsubmission) {
                ...
                foreach ($members as $member) {
                    $userslist[] = $this->output->action_link(new moodle_url('/user/view.php', array('id' => $member->id, 'course'=>$status->courseid)), fullname($member, $status->canviewfullnames));
                }
                if (count($userslist) > 0) {
                    $submissionsummary .= $this->output->container(get_string('userswhoneedtosubmit', 'assign', join(', ', $userslist)));
                }
