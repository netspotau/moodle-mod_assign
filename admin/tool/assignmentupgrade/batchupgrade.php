...
while ($current < count($assignmentids)) {
    list($summary, $success, $log) = tool_assignmentupgrade_upgrade_assignment($assignmentids[$current]);
    $current += 1;
    echo $renderer->heading(get_string('upgradeprogress', 'tool_assignmentupgrade', array('current'=>$current, 'total'=>count($assignmentids))), 3);
